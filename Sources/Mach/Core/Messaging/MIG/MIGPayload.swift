...
extension Mach {
    ...
    public struct MIGPayloadPrefixedWithNDR<InnerPayload: BitwiseCopyable>: Mach.MIGPayload {
        ...
        public static func fromPayloadData(_ data: Data) -> Self? {
            let requiredSize = MemoryLayout<NDR_record_t>.size + MemoryLayout<InnerPayload>.size
            guard data.count >= requiredSize else { return nil }

            return data.withUnsafeBytes { buffer in
                let ndr = buffer.load(fromByteOffset: 0, as: NDR_record_t.self)
                let innerPayload = buffer.load(
                    fromByteOffset: MemoryLayout<NDR_record_t>.size,
        ...
        public var payloadData: Data {
            let totalSize = MemoryLayout<NDR_record_t>.size + MemoryLayout<InnerPayload>.size
