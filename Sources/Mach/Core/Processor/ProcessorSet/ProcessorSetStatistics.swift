...
extension Mach {
    /// A type of processor set statistics.
...
extension Mach.ProcessorSet {
    ...
    public func getStatistics<DataType: BitwiseCopyable>(
        _ statistics: Mach.ProcessorSetStatisticsFlavor, as type: DataType.Type
    ) throws -> DataType {
        try Mach.callWithCountInOut(type: type) {
            (array: processor_set_info_t, count) in
            processor_set_statistics(self.name, statistics.rawValue, array, &count)
        }
    }
}
