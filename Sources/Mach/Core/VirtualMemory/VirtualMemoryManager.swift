    ...
    extension Mach {
        /// A kernel space tag for virtual memory.
        /// - Note: Normally, these would only be used in the kernel. However, the
        ///     range-creation API appears to use them in its recipe format.
        public struct VMKernelTag: KassHelpers.NamedOptionEnum {
            /// Represents a raw kernel memory tag with an optional name.
            public init(name: String?, rawValue: Int32) {
                self.name = name
                self.rawValue = rawValue
            }
        ...
        /// A user space tag for virtual memory.
        public struct VMUserTag: KassHelpers.NamedOptionEnum {
            private struct C: KassHelpers.Namespace {
                @available(macOS, introduced: 26.0)
                static let mach_vm_tag_describe: @convention(c) (UInt32) -> UnsafePointer<CChar> =
    ...
    extension mach_vm_range_recipe_v1_t {
        public var vmTag: Mach.VMKernelTag {
            get { return Mach.VMKernelTag(rawValue: Int32(self.vm_tag)) }
