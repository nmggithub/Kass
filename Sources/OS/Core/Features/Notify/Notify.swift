...
extension OS {
    ...
    public struct Notify: KassHelpers.Namespace {
        ...
        public struct NotificationName {
            ...
            public func post() throws {
                try Notify.call(notify_post(name))
            }
            ...
            public func register(
                ...
                try Notify.call(notify_register_dispatch(name, &token, queue, handler))
            ...
            public func register() throws -> NotificationToken {
                var token: Int32 = NOTIFY_TOKEN_INVALID
                try Notify.call(notify_register_check(name, &token))
                return NotificationToken(rawValue: token)
            }
            ...
            public func register(signal: Int32) throws -> NotificationToken {
                var token: Int32 = NOTIFY_TOKEN_INVALID
                try Notify.call(notify_register_signal(name, signal, &token))
                return NotificationToken(rawValue: token)
            }
            ...
            public func register(machPort: inout Mach.Port?, flags: NotifyFlags) throws
                -> NotificationToken
            {
                var portName = machPort?.name ?? mach_port_name_t(MACH_PORT_NULL)
                var token: Int32 = NOTIFY_TOKEN_INVALID
                try Notify.call(
                    notify_register_mach_port(name, &portName, flags.rawValue, &token))
            ...
            ///     file descriptor.
            public func register(fileDescriptor fd: inout Int32, flags: NotifyFlags) throws
                -> NotificationToken
            {
                var token: Int32 = NOTIFY_TOKEN_INVALID
                try Notify.call(
                    notify_register_file_descriptor(name, &fd, flags.rawValue, &token))
                return NotificationToken(rawValue: token)
            }
        }
